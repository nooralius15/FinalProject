...
def render_student_panel(
    ...
    if roster.empty:
        st.warning(_t("Secilen danismana ait ogrenci bulunamadi."))
        return
    ...
    if fixed_student_no:
        ...
        selected = st.selectbox(_t("Öğrenci"), list(student_map.keys()))
    ...
    student_role = role_row["role"] if role_row else "Üye"
    ...
    st.markdown(
        ...
        <div class="dm-hero-banner">
            <div style="display:flex;align-items:flex-start;gap:1rem;">
                <div style="width:52px;height:52px;background:rgba(255,215,0,0.15);border:2px solid var(--hero-title);
                            border-radius:50%;display:flex;align-items:center;justify-content:center;
                            font-size:1.4rem;flex-shrink:0;">{role_icon}</div>
                <div style="flex:1;min-width:0;">
                    <div style="color:var(--hero-title);font-size:0.68rem;font-weight:700;letter-spacing:.1em;
                                text-transform:uppercase;">{_t('Öğrenci Paneli')}</div>
                    <div style="color:var(--hero-text);font-size:1.2rem;font-weight:800;margin:0.1rem 0;">{student_name}</div>
                    <div style="color:var(--hero-sub);font-size:0.75rem;">{student_no} &nbsp;·&nbsp; {project_name}</div>
                    <div style="margin-top:0.4rem;display:flex;gap:0.4rem;flex-wrap:wrap;">
                        ...
                        <span style="background:rgba(255,255,255,0.1);color:var(--hero-sub);border-radius:999px;
                                     padding:.1em .6em;font-size:.7rem;">👨&#8205;🏫 {advisor_name}</span>
                    </div>
                </div>
                <div style="text-align:right;flex-shrink:0;">
                    <div style="color:var(--hero-title);font-size:1.8rem;font-weight:900;line-height:1;">%{int(my_pct)}</div>
                    <div style="color:var(--hero-sub);font-size:0.68rem;">{_t('kişisel ilerleme')}</div>
    ...
    c2.metric(_t("Proje Tamamlanma"), f"%{int(proj_pct)}")
    ...
    if my_tasks.empty:
        st.info(_t("Size atanmis gorev yok."))
    else:
        ...
        for _, t in ordered.iterrows():
            ...
            <tr class="dm-table-row">
                <td style="padding:.4rem .6rem;font-size:.72rem;color:var(--text-color,#64748b);opacity:.7;white-space:nowrap;">{ms_label}</td>
                <td style="padding:.4rem .6rem;font-size:.82rem;color:var(--text-color,#1e293b);">{t['title']}</td>
        ...
        st.markdown(
            f"""<div style="border:1px solid var(--border,#e2e8f0);border-radius:10px;overflow:hidden;margin-bottom:.75rem;box-shadow:0 1px 4px rgba(0,0,0,.05);">
            <table style="width:100%;border-collapse:collapse;">
            <thead><tr style="background:var(--table-header-bg,#0a2342);">
            <th class="dm-table-header">Milestone</th>
            <th class="dm-table-header">Görev</th>
            <th class="dm-table-header">Durum</th>
            <th class="dm-table-header">Deadline</th>
        ...
        if task_row is None:
            st.markdown(
                f"""<div style="background:var(--success-bg,#dcfce7);border:1px solid var(--success,#16a34a);border-radius:10px;
                              padding:.75rem 1rem;font-size:.88rem;font-weight:600;color:var(--success,#14532d);">
            ...
            st.markdown(
                f"""<div style="background:var(--warning-bg,#fef3c7);border-left:4px solid var(--warning,#d97706);border-radius:4px 10px 10px 4px;
                               padding:.6rem .9rem;margin-bottom:.5rem;font-size:.82rem;color:var(--text-color,#78350f);">
            ...
            with st.form(f"student_task_form_{student_no}"):
                st.markdown(
                    "<div style='font-size:0.75rem;color:var(--text-color,#64748b);opacity:.7;margin-bottom:.3rem;'>"
                    "💡 Önceki milestone tamamlanmadan sonraki milestone görevine geçilemez.</div>",
                    unsafe_allow_html=True,
                )
                status = st.selectbox(_t("Durum güncelle"), status_options, index=status_idx, format_func=status_tr)
                evidence = st.text_input(_t("Kanıt linki (repo, rapor, vs.)"), value=task_row["evidence_link"] or "")
            ...
            if save_task:
                ...
                st.success(_t("Goreviniz guncellendi.")) if ok else st.error(msg)
                if ok:
                    st.rerun()
    ...
    if current_task is None:
        ...
        with st.form(f"weekly_form_{student_no}"):
            st.text_input(_t("İlgili görev"), value=current_task_label, disabled=True)
            ...
            with col_b:
                ...
                evidence_link = st.text_input(_t("Kanıt linki"))
    ...
    if my_weekly_df.empty:
        st.info(_t("Henuz haftalik giris yapilmamis."))
    else:
        st.dataframe(
    ...
    if not compare_df.empty:
        ...
        st.markdown(
            f"""
            <div class="dm-leaderboard" style="display:flex;align-items:center;gap:1rem;">
                <span style="font-size:2rem;">{podium}</span>
                <div>
                    <div class="dm-text-primary" style="font-weight:800;font-size:1.1rem;">{rank} / {total}</div>
                    <div class="dm-text-secondary" style="font-size:.78rem;">{project_name} &nbsp;·&nbsp; %{pct} {_t('tamamlandı')}</div>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    section_header("🤖", _t("AI Kişisel Koç"), _t("Yapay zeka çalışmanı analiz eder ve sana özel tavsiyeler verir"))
    col_ai1, col_ai2 = st.columns([3, 1])
    with col_ai2:
        run_student_ai = st.button("✨ Çalışmamı Analiz Et", key=f"student_ai_run_{student_no}", type="primary")
    with col_ai1:
        st.caption(_t("Ollama (Gemini Flash) görevlerini, raporlarını ve danışman geri bildirimlerini analiz eder."))

    ai_key = f"student_ai_result_{student_no}_{project_name}"
    if st.session_state.get(ai_key):
        from ui_helpers import render_ai_insight_card
        render_ai_insight_card(st.session_state[ai_key])

    if run_student_ai:
        from ai_client import check_ollama, build_student_prompt, generate
        import pandas as pd
        if not check_ollama():
            ...
            my_weekly_df2 = fetch_weekly_updates_for_project(conn, project_name, student_no)
            weekly_ai = [
                {"week_start": str(r["week_start"]), "completed": str(r["completed"] or "")}
                for _, r in my_weekly_df2.head(5).iterrows()
            ] if not my_weekly_df2.empty else []
            fb_df = fetch_feedbacks(conn, project_name)
            fb_ai = [
                {"feedback": str(r["feedback"])}
                for _, r in fb_df.head(3).iterrows()
            ] if not fb_df.empty else []
            # Compute rank
            rank_val = None
            total_g = None
            if not compare_df.empty:
                mine_row = compare_df[compare_df["Proje"] == project_name]
                if not mine_row.empty:
                    rank_val = int(mine_row.iloc[0]["Sıra"])
                    total_g = len(compare_df)
            with st.spinner("🤖 AI seni analiz ediyor..."):
