...
def render_advisor_panel(conn, advisor_name: str, roster: pd.DataFrame) -> None:
    # ── Page title ────────────────────────────────────────────────────────────
    panel_title = _t('Danışman Paneli')
    st.markdown(
        f"""
        <div class="dm-hero-banner">
            <div style="color:var(--hero-title);font-size:0.7rem;font-weight:700;letter-spacing:.1em;
                        text-transform:uppercase;margin-bottom:0.2rem;">{panel_title}</div>
            <div style="color:var(--hero-text);font-size:1.3rem;font-weight:800;">{advisor_name}</div>
        </div>
        """,
    ...
    if not projects:
        st.warning(_t("Bu danismana ait grup/ogrenci kaydi yok."))
        return
    ...
    # ── Project cards grid ────────────────────────────────────────────────────
    section_header("📁", _t("Proje Genel Bakışı"), f"{len(projects)} {_t('aktif proje')}")
    ...
    # ── Summary table ─────────────────────────────────────────────────────────
    if not summary_df.empty:
        section_header("📊", _t("Proje Özeti"), _t("Tüm projelerin tamamlanma, gecikme ve risk durumu"))
        display = summary_df.copy().sort_values(["Tamamlanma %", "Geciken Gorev"], ascending=[False, True])
        st.dataframe(display,width="stretch", hide_index=True)

    # ── AI Project Analysis ────────────────────────────────────────────────────
    section_header("🤖", _t("AI Proje Analizi"), _t("Yapay zeka destekli özet ve risk değerlendirmesi"))
    col_ai1, col_ai2 = st.columns([3, 1])
    with col_ai2:
        run_ai = st.button(_t("✨ Tüm Projeleri Analiz Et"), key="advisor_ai_run", type="primary")
    with col_ai1:
        st.caption(_t("Ollama (Gemini Flash) tüm proje verilerini analiz eder ve danışmana özel bir rapor üretir."))

    # Show cached result if available
    if st.session_state.get("advisor_ai_result"):
        from ui_helpers import render_ai_insight_card
        render_ai_insight_card(st.session_state["advisor_ai_result"])

    if run_ai:
        from ai_client import check_ollama, build_advisor_prompt, generate
        if not check_ollama():
            st.error(_t("⚠️ Ollama çalışmıyor. Lütfen terminal'de `ollama serve` komutunu çalıştırın."))
        else:
            # Build projects_data list for the prompt
            projects_data_ai = []
            for project in projects:
                grp = roster[roster["project_name"] == project]
                leader_no = get_leader(conn, project)
                ...
                if leader_no:
                    ...
                    leader_name = str(hit.iloc[0]["student_name"]) if not hit.empty else leader_no
                risk_row = summary_df[summary_df["Proje"] == project] if not summary_df.empty else pd.DataFrame()
                pct = float(risk_row.iloc[0]["Tamamlanma %"]) if not risk_row.empty else 0.0
                overdue = int(risk_row.iloc[0]["Geciken Gorev"]) if not risk_row.empty else 0
                risk = str(risk_row.iloc[0]["Risk"]) if not risk_row.empty else "Orta"
                activity = int(risk_row.iloc[0]["Son 14 Gun Aktivite"]) if not risk_row.empty else 0
                projects_data_ai.append({
                    "name": project,
                    "leader": leader_name,
                    "members": len(grp),
                    "completion_pct": pct,
            ...
            with st.spinner(_t("🤖 AI analiz yapıyor...")):
                prompt = build_advisor_prompt(advisor_name, projects_data_ai)
                result = generate(prompt)
            st.session_state["advisor_ai_result"] = result
            from ui_helpers import render_ai_insight_card
            render_ai_insight_card(result)
            st.rerun()


    section_header("🔍", _t("Öğrenci Arama"), _t("Ad veya numara ile arayın"))
    ...
    if search_query.strip():
        ...
        if matches.empty:
            st.warning(f'"{search_query}" {_t("ile eslesen ogrenci bulunamadi.")}')
        else:
            ...
            if stu_rows.empty:
                st.error(_t("Ogrenci kaydi bulunamadi."))
            else:
                _render_student_detail(conn, roster, stu_rows, picked_no)

    # ── Leader assignment ─────────────────────────────────────────────────────
    section_header("👑", _t("Proje Lideri Atama"))
    ...
    with st.form("leader_assign_form"):
        label = st.selectbox(_t("Lider adayı"), labels, index=default_idx)
    ...
    if submitted:
        ...
        upsert_role(conn, project_name, leader_no, "Lider", _t("Danisman tarafindan atanmis lider"))
        st.success(_t("Lider guncellendi."))
        st.rerun()

    # ── CSV upload ────────────────────────────────────────────────────────────
    section_header("📤", _t("CSV Yükleme"), _t("Öğrenci listesini CSV dosyasıyla güncelleyin"))
    with st.expander(_t("CSV Yükle / Güncelle")):
        ...
        if uploaded_csv is not None:
            try:
                ...
                st.caption(f"{len(new_roster)} {_t('öğrenci kaydı bulundu.')}")
                if st.button("✅ Öğrenci Listesini Güncelle", key="apply_csv_btn"):
                    ...
                    st.cache_data.clear()
                    st.success(f"{count} {_t('ogrenci kaydi guncellendi.')}")
                    st.rerun()
            except Exception as e:
                st.error(f"{_t('CSV okuma hatasi:')} {e}")

    # ── Add single student ────────────────────────────────────────────────────
    section_header("➕", _t("Tek Öğrenci Ekleme"))
    _render_add_student_form(conn, roster, advisor_name)

    # ── Password reset ────────────────────────────────────────────────────────
    section_header("🔑", _t("Şifre Sıfırlama"))
    _render_password_reset(conn, roster)

    # ── Project detail / task management ─────────────────────────────────────
    section_header("🗂️", _t("Proje Detayı & Görev Yönetimi"))
...
def _render_student_detail(conn, roster, stu_rows, picked_no: str) -> None:
    ...
    st.markdown(
        f"""
        <div class="dm-card-detail">
            <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:0.6rem;">
                <div style="width:42px;height:42px;background:var(--hero-bg);
                            border-radius:50%;display:flex;align-items:center;justify-content:center;
                            font-size:1.1rem;flex-shrink:0;">🎓</div>
                <div>
                    <div class="dm-text-primary" style="font-weight:700;font-size:1rem;">{stu_name}</div>
                    <div class="dm-text-muted" style="font-size:0.73rem;">{picked_no} &nbsp;·&nbsp; {stu_project}</div>
                </div>
            ...
            <div class="dm-text-secondary" style="font-size:0.77rem;border-top:1px solid var(--border,#e2e8f0);padding-top:0.5rem;">
    ...
    if not my_tasks.empty:
        ...
        task_table["Milestone"] = task_table["milestone_key"].map(MILESTONE_LABELS).apply(_t)
        ...
        st.info(_t("Bu ogrenciye atanmis gorev yok."))
...
def _render_add_student_form(conn, roster, advisor_name: str) -> None:
    ...
    with st.expander(_t("➕ Yeni Öğrenci Ekle")):
        existing_projects = sorted(roster["project_name"].unique().tolist()) if not roster.empty else []
        with st.form("add_single_student_form"):
            new_student_no = st.text_input(_t("Öğrenci No"), placeholder="2001234567")
            new_student_name = st.text_input(_t("Ad Soyad"))
            use_existing_project = st.checkbox(_t("Mevcut bir projeye ekle"), value=True)
            if use_existing_project and existing_projects:
                ...
                new_project_name = st.text_input(_t("Yeni proje adı"))
        ...
        if add_student_submit:
            if not new_student_no.strip():
                st.error(_t("Ogrenci no bos olamaz."))
            elif not new_student_name.strip():
                st.error(_t("Ogrenci adi bos olamaz."))
            elif not new_project_name.strip():
                st.error(_t("Proje adi bos olamaz."))
            else:
                ...
                if int(existing_check) > 0:
                    st.error(f"{new_student_no} {_t('numarali ogrenci zaten')} '{new_project_name}' {_t('projesinde kayitli.')}")
                else:
                    ...
                    st.cache_data.clear()
                    st.success(f"{new_student_name} ({new_student_no}) {_t('basariyla')} '{new_project_name}' {_t('projesine eklendi.')}")
                    st.rerun()


def _render_password_reset(conn, roster) -> None:
    with st.expander(_t("🔑 Şifre Sıfırla")):
        st.markdown(
            "<div style='font-size:0.78rem;color:var(--text-color,#64748b);opacity:.7;margin-bottom:0.5rem;'>"
        ...
        if reset_role == "student":
            ...
            if student_list.empty:
                st.info(_t("Sifirlanacak ogrenci yok."))
            else:
                reset_options = {
                    f"{r['student_name']} ({r['student_no']})": str(r["student_no"])
                ...
                if st.button("🔑 Şifreyi Sıfırla", key="pwd_reset_btn_student"):
                    ...
                    st.success(f"{selected_reset_user} {_t('sifresi sifirlandi.')}") if ok else st.error(_t("Kullanici bulunamadi."))
        else:
            from db import fetch_df
            ...
            if advisor_list.empty:
                st.info(_t("Sifirlanacak danisman yok."))
            else:
                ...
                if st.button("🔑 Şifreyi Sıfırla", key="pwd_reset_btn_advisor"):
                    ...
                    st.success(f"{selected_adv_reset} {_t('sifresi sifirlandi.')}") if ok else st.error(_t("Kullanici bulunamadi."))
...
def _render_project_detail(conn, roster, projects, advisor_name: str) -> None:
    ...
    if not tasks_df.empty:
        table = tasks_df.copy()
        table["Milestone"] = table["milestone_key"].map(MILESTONE_LABELS).apply(_t)
    ...
    if not tasks_df.empty:
        ...
        render_active_task_card(adv_task_row, _t(MILESTONE_LABELS.get(str(adv_task_row["milestone_key"]), "")))
        ...
        with st.form(f"advisor_task_update_form_{detail_project}"):
            adv_new_status = st.selectbox(_t("Yeni durum"), adv_status_options, index=adv_status_idx, format_func=status_tr)
            adv_evidence = st.text_input(_t("Kanıt linki"), value=adv_task_row["evidence_link"] or "")
        ...
        if adv_task_submit:
            ...
            st.success(_t("Gorev guncellendi.")) if ok else st.error(msg)
            if ok:
                st.rerun()
    ...
    with st.form("feedback_form"):
        ...
        action_item = st.text_input(_t("Aksiyon maddesi"))
        revision_required = st.checkbox(_t("Revizyon gerekli"))
        fb_submit = st.form_submit_button("💾 Kaydet",width="stretch")
    if fb_submit:
        if not feedback.strip():
            st.error(_t("Geri bildirim bos olamaz."))
        else:
            add_feedback(conn, detail_project, advisor_name, feedback, action_item, revision_required)
            st.success(_t("Geri bildirim kaydedildi."))
            st.rerun()
