...
def render_ai_insight_card(response_text: str, model_name: str = "llama3.2:3b") -> None:
    ...
    if in_list:
        html_lines.append("</ul>")
    body = "\n".join(html_lines)
    short_model = model_name.replace(":latest", "").replace("gemini-3-flash-preview", "Gemini Flash").replace("llama3.2:3b", "Llama 3.2")
    card = (
