...
def render_ai_insight_card(response_text: str, model_name: str = "gemini-3-flash-preview") -> None:
    ...
    if in_list:
        html_lines.append("</ul>")
    body = "\n".join(html_lines)
    short_model = model_name.replace(":latest", "").replace("gemini-3-flash-preview", "Gemini Flash")
    card = (
