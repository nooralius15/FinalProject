...
def render_milestone_progress(tasks_df: pd.DataFrame) -> None:
    st.markdown(f"#### 📊 {_t('Milestone İlerlemesi')}")
    ...
    for col, (milestone_key, label) in zip(cols, MILESTONES):
        ...
        if percent >= 100:
            bar_color, bg = "#16a34a", "var(--success-bg, #dcfce7)"
        elif percent >= 50:
            bar_color, bg = "#0f4c81", "var(--info-bg, #dbeafe)"
        elif percent > 0:
            bar_color, bg = "#d97706", "var(--warning-bg, #fef3c7)"
        else:
            bar_color, bg = "#94a3b8", "var(--secondary-background-color, #f1f5f9)"

        short_label = milestone_key  # M1-M6
        col.markdown(
            f"""
            <div class="dm-milestone-card" style="background:{bg};">
                <div style="font-size:0.65rem;font-weight:700;color:var(--text-color, #475569);
                            text-transform:uppercase;letter-spacing:.05em;margin-bottom:0.2rem;opacity:.8;">{short_label}</div>
                <div style="font-size:1.3rem;font-weight:800;color:{bar_color};">{int(percent)}%</div>
                <div style="font-size:0.67rem;color:var(--text-color, #475569);opacity:.7;margin-top:0.1rem;">{done}/{total} {_t('Gorev')}</div>
                <div style="background:var(--border, #e2e8f0);border-radius:999px;height:5px;margin-top:0.4rem;overflow:hidden;">
